...
``` r
filter(litters_df, pups_born_alive <= 5) # or >, <, add equals
```
...
``` r
filter(litters_df, pups_born_alive < 4 | pups_born_alive > 8) # | means "or"
```
...
In the pups dataset, keep only rows where the PD walk is less than 11
and sex is 2
...
```
    ...
    ## # A tibble: 313 × 7
    ##    litter_number   sex pd_ears pd_eyes pd_pivot pd_walk pd_walk_7
...
litters_df |> 
  ...
  lm(gd18_weight ~ gd0_weight, data = _) # the _ tells R that the df is litters_df
```
...
``` r
# select will always give you a data frame; if you have a data frame you should never be pulling stuff out of it
```

never do `$` –\> takes things out of the dataframe for manipulations –\>
...
``` r
 # view() #don't keep view active when knitting
```
